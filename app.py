...
.big-title {
    font-size: 55px; /* ใหญ่ขึ้นจากเดิม 31px */
    font-weight: 800; 
    color: #F8FAFC;
    letter-spacing: -1.5px; /* ทำให้ตัวอักษรชิดกันนิดนึงแบบสปอร์ต */
    line-height: 1.1;
    margin-bottom: 5px;
    text-transform: uppercase;
    text-shadow: 0px 0px 20px rgba(248, 250, 252, 0.2);
}
.sub {.sub {
    font-size: 22px; /* ใหญ่ขึ้นจากเดิม 14px */
    color: #94A3B8;
    font-weight: 400;
    margin-bottom: 20px;
}
...
with tabs[0]:
    ...
    st.write("""
    **Dataset 1: Formula1_Pitstop_Data_1950-2024_all_rounds.csv** (Structured)  
    ที่มา: Kaggle - https://www.kaggle.com/datasets/akashrane2609/formula-1-pit-stop-dataset  
    Feature สำคัญ: Season, Circuit, Driver, Constructor, PitStopCount, StintLength, AggressionScore, TrackTemp, AirTemp, Humidity, WindSpeed  
    ความไม่สมบูรณ์: มี missing values ใน weather และ tire compound

    **Dataset 2: f1_enhanced_dataset_for_analysis.csv** (Structured)  
    ที่มา: Kaggle - https://www.kaggle.com/datasets/usman136/formula-1-race-dataset-for-predictive-analysis  
    Feature สำคัญ: GridPosition, QualifyingPosition, TireStrategy, Weather, StartingPosition  
    ความไม่สมบูรณ์: มี missing values และ class imbalance (DNF น้อย)
    """)
...
with tabs[2]:
    ...
    st.write("""
    **การเตรียมข้อมูล**: Clean missing values, normalization, Target = Top10 (binary)  
    **ทฤษฎี**: Feedforward Neural Network (MLP) ด้วย 3 hidden layers + Dropout 0.2 + Sigmoid output  
    **ขั้นตอน**: สร้าง Sequential model → Compile ด้วย Adam + binary_crossentropy → Train 30 epochs  
    **แหล่งอ้างอิง**: TensorFlow / Keras + Kaggle F1 datasets
    """)
